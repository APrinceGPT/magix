...
const LESSONS = [
  ...
  // ══════════════════════════════
  // ADDITIONAL LESSONS — reaching 43 total
  // ══════════════════════════════

  // FOUNDATIONS — 2 more
  {
    _id: 'lesson-card-handling-basics',
    _type: 'lesson',
    title: 'Card Handling Basics',
    slug: { _type: 'slug', current: 'card-handling-basics' },
    category: 'foundations',
    level: 'complete-beginner',
    tagline: 'The invisible language of the hands. Learn to speak it.',
    overview: "Before you perform a single trick, you need to be comfortable with cards in your hands — picking them up, turning them over, tabling them, and handling them fluidly. Awkward card handling is immediately visible and destroys credibility before a trick begins.",
    estimatedTime: 10,
    featured: false,
    order: 9,
    steps: [
      { _key: 'chb-s1', stepNumber: 1, title: 'Picking Up the Deck', instruction: "Always pick up the deck from the sides — thumb on one long edge, fingers on the other. Never grab from the top and bottom, which exposes the faces and looks clumsy.", fingerNote: "The way you pick up the deck sets the tone. Smooth and deliberate from the very first moment.", cardAnimation: 'cut' },
      { _key: 'chb-s2', stepNumber: 2, title: 'Turning Cards Over', instruction: "When turning a card face-up on the table, use a single finger to pivot it from a corner — not a two-handed flip. One finger, one clean rotation, card face-up. Fast, elegant, professional.", cardAnimation: 'reveal' },
      { _key: 'chb-s3', stepNumber: 3, title: 'Tabling the Deck', instruction: "Place the deck on the table in one motion — don't fidget or adjust after placing. Set it down square and leave it. The absence of unnecessary fiddling reads as confidence.", },
      { _key: 'chb-s4', stepNumber: 4, title: 'The Casual Fan', instruction: "Holding a few cards in a loose fan in one hand while gesturing with the other is a basic piece of stage business. Practice holding 5–10 cards in a relaxed fan while doing other things — talking, gesturing, thinking.", cardAnimation: 'fan', fingerNote: "A comfortable casual fan is one of the clearest signals that you handle cards every day. It communicates expertise without performing." },
    ],
    commonMistakes: [
      { _key: 'chb-m1', mistake: 'Fumbling when picking up dropped cards', fix: "When a card falls, pick it up in one smooth motion without comment or apology. Treating it as a non-event reads as confidence." },
      { _key: 'chb-m2', mistake: 'Two-handed everything', fix: "Develop the habit of handling individual cards one-handed. Two-handed card handling looks like you haven't done it much." },
    ],
    practiceDrill: { title: 'The Handling Circuit', description: "Pick up the deck, fan it, close it, table it, pick it up again — all one-handed, all smooth. Repeat as a warm-up before every practice session.", reps: '5 minutes at the start of each practice' },
    performanceContext: "Card handling basics are what the audience sees before you do anything. A magician who handles cards effortlessly already has the audience's trust — and you haven't done a single trick yet.",
  },

  {
    _id: 'lesson-false-cut',
    _type: 'lesson',
    title: 'False Cut',
    slug: { _type: 'slug', current: 'false-cut' },
    category: 'foundations',
    level: 'beginner',
    tagline: 'The cut that changes nothing — invisibly.',
    overview: "A false cut appears to cut the deck into multiple packets and reassemble them randomly — but the deck ends up in exactly the same order it started. False cuts are among the most powerful tools in card magic because spectators inherently trust cuts as a randomising action.",
    estimatedTime: 20,
    featured: false,
    order: 10,
    steps: [
      { _key: 'fc-s1', stepNumber: 1, title: 'The Tabled False Cut', instruction: "Place the deck on the table. Cut the top third to the left (A). Cut the middle third to the right (B). Place the original bottom third (C) on top of A. Then place B on top of C+A. Then place the original top portion back. Despite three visible cuts, the deck is in original order.", cardAnimation: 'cut', fingerNote: "The reassembly order is the key: the packets must return in a specific sequence that reverses the apparent cuts." },
      { _key: 'fc-s2', stepNumber: 2, title: 'The One-Handed False Cut', instruction: "Hold the deck in your right hand. Your index finger levers the bottom half forward. That packet swings past the top half, and you let it fall into your left hand — then drop the right-hand packet on top. Net result: the deck is cut and reassembled, but the order is restored.", cardAnimation: 'shuffle', fingerNote: "Practise the timing of the swing so the bottom half clearly 'passes' the top half visibly — that visual is what makes it look fair." },
      { _key: 'fc-s3', stepNumber: 3, title: 'Combining False Cuts', instruction: "The most convincing approach: perform two different false cuts in quick succession. The cumulative 'fairness' of multiple visible cuts overwhelms any suspicion. In reality, every cut returns the deck to original order.", },
    ],
    commonMistakes: [
      { _key: 'fc-m1', mistake: 'Performing the false cut too quickly', fix: "A rushed cut looks nervous. Slow, deliberate cuts with pauses between look completely fair." },
      { _key: 'fc-m2', mistake: 'Mixing real and false cuts accidentally', fix: "Know which cuts you're performing at all times. A real cut inserted into a false cut sequence scrambles your deck." },
    ],
    practiceDrill: { title: 'Order Preservation Test', description: "Note the top five cards of the deck. Perform your full false cut sequence. The top five cards should be in identical order. Test every session.", reps: '20 rounds — zero order disturbance' },
    performanceContext: "Offer a false cut after every trick as a reset. 'Let me cut the cards to mix them up again.' The audience believes the deck is thoroughly randomised. They're wrong.",
  },

  // SHUFFLES — 2 more
  {
    _id: 'lesson-milk-build',
    _type: 'lesson',
    title: 'Milk Build Shuffle',
    slug: { _type: 'slug', current: 'milk-build-shuffle' },
    category: 'shuffles',
    level: 'intermediate',
    tagline: 'Build any hand from a shuffled deck. Invisibly.',
    overview: "The milk build is an overhand shuffle technique that allows you to assemble specific cards onto the top of the deck by 'milking' them from the top and bottom of the packet during the shuffle. It looks like a normal overhand shuffle but delivers a completely stacked top portion.",
    estimatedTime: 35,
    featured: false,
    order: 9,
    steps: [
      { _key: 'mb-s1', stepNumber: 1, title: 'The Milk Action', instruction: "Hold the deck in overhand shuffle grip. As your right hand lifts the deck away for the first shuffle action, your left thumb takes the top card AND your left fingers take the bottom card simultaneously — 'milking' one from each end. These two cards are now on the left-hand packet.", cardAnimation: 'shuffle', fingerNote: "The milk action must look identical to a normal first shuffle movement. The two cards being taken simultaneously must not cause any change in the hand's appearance." },
      { _key: 'mb-s2', stepNumber: 2, title: 'Building the Stack', instruction: "Repeat the milk action with each shuffle: take one from the top and one from the bottom of the right-hand packet. Over several milks, you're pulling cards from both ends of the deck and stacking them together on the left-hand packet.", },
      { _key: 'mb-s3', stepNumber: 3, title: 'The Application', instruction: "Before performing, secretly place your target cards alternating at top and bottom of the deck. After several milk build shuffles, all target cards are together at the top of the deck — ready to be dealt as a winning hand or revealed as a prediction.", cardAnimation: 'reveal', fingerNote: "The number of milk actions equals the number of cards you want to collect. Four milk actions builds four cards to the top." },
    ],
    commonMistakes: [
      { _key: 'mb-m1', mistake: 'The double take being visible', fix: "The simultaneous top-and-bottom take must feel like a single motion. Practice until your thumb and fingers move as one unit." },
      { _key: 'mb-m2', mistake: 'Losing track of how many milks performed', fix: "Count silently. Each milk is one card delivered. Know exactly how many you need before you begin." },
    ],
    practiceDrill: { title: 'Four-Ace Build', description: "Place the four Aces alternating at top and bottom (Ace top, Ace bottom, Ace second from top, Ace second from bottom). Perform two milk build shuffles. All four Aces should be at the top.", reps: '25 rounds — all four Aces on top every time' },
    performanceContext: "The milk build is the foundation of gambling demonstrations — dealing yourself a royal flush 'after the shuffle.' Performed naturally, it's undetectable even to experienced observers.",
  },

  {
    _id: 'lesson-zarrow-shuffle',
    _type: 'lesson',
    title: 'Zarrow Shuffle',
    slug: { _type: 'slug', current: 'zarrow-shuffle' },
    category: 'shuffles',
    level: 'professional',
    tagline: 'A perfect riffle shuffle that leaves the deck completely unchanged.',
    overview: "The Zarrow shuffle, created by Herb Zarrow in the 1950s, is the most deceptive false shuffle in card magic. It looks and sounds exactly like a genuine riffle shuffle — but the deck ends up in precisely the same order it started. It is the professional standard for false shuffling.",
    estimatedTime: 90,
    featured: false,
    order: 10,
    steps: [
      { _key: 'zs-s1', stepNumber: 1, title: 'The Setup', instruction: "The Zarrow begins like a genuine table riffle shuffle: split the deck into two halves, riffle the short edges together. The weave looks perfectly genuine because the early part of the shuffle is genuine.", cardAnimation: 'shuffle' },
      { _key: 'zs-s2', stepNumber: 2, title: 'The Secret', instruction: "As you push the two halves together, instead of interlacing them, your left fingers secretly push the entire left-hand packet underneath the right-hand packet. The right packet rides on top, and the two are stacked — not woven.", fingerNote: "The push-under happens during the bridge action — when the cards are arched. The arching covers the lateral movement of the left packet.", cardAnimation: 'cut' },
      { _key: 'zs-s3', stepNumber: 3, title: 'The Bridge Completion', instruction: "Complete the bridge exactly as normal — the arch collapses and the deck is 'squared.' From every visible angle, a genuine riffle shuffle has occurred. The deck is in original order.", },
      { _key: 'zs-s4', stepNumber: 4, title: 'The Sound', instruction: "The Zarrow sounds slightly different from a genuine shuffle because the cards don't actually weave. Practice until the sound is identical — this is the hardest aspect of the shuffle to perfect.", fingerNote: "Create a woven sound by letting a few genuine cards riffle at the start and end, sandwiching the false section." },
    ],
    commonMistakes: [
      { _key: 'zs-m1', mistake: 'Left packet not going fully under', fix: "The entire left half must travel under the right. A partial push leaves a visible block of cards at the edge." },
      { _key: 'zs-m2', mistake: 'Unnatural bridge speed', fix: "The bridge and square must happen at normal shuffle speed. A slow, careful bridge looks suspicious." },
    ],
    practiceDrill: { title: 'New Deck Order Test', description: "Start with a new deck in factory order. Perform three Zarrow shuffles. The deck should still be in factory order. Use this as your daily pass/fail test.", reps: 'Daily — three shuffles, check factory order' },
    performanceContext: "The Zarrow shuffle is the ultimate credibility tool. When you offer to let the audience shuffle and they decline, you perform three apparently thorough riffle shuffles — and everything is exactly as you arranged it.",
  },

  // SLEIGHTS — 1 more
  {
    _id: 'lesson-braue-reversal',
    _type: 'lesson',
    title: 'Braue Reversal',
    slug: { _type: 'slug', current: 'braue-reversal' },
    category: 'sleights',
    level: 'intermediate',
    tagline: 'Secretly reverse any card in the deck. No one sees it happen.',
    overview: "The Braue reversal is a technique for secretly turning a card face-up inside the face-down deck — or vice versa. It's the engine behind many 'reversed card' discoveries where a chosen card is found face-up in a face-down deck, apparently turning itself around.",
    estimatedTime: 30,
    featured: false,
    order: 9,
    steps: [
      { _key: 'br-s1', stepNumber: 1, title: 'The Setup', instruction: "You need a break below the card you want to reverse — your target card is directly above the break. The deck is in dealer's grip in your left hand.", fingerNote: "The break must be maintained cleanly throughout the reversal. If the break is lost, the reversal fails.", cardAnimation: 'cut' },
      { _key: 'br-s2', stepNumber: 2, title: 'The Reverse Action', instruction: "Your right hand comes over the deck and lifts the upper portion (above the break) into right-hand biddle grip. Simultaneously, your left thumb pushes the target card (now top of the lower packet) face-up against the bottom of the right-hand packet. The right-hand packet is then replaced on the left-hand packet.", cardAnimation: 'shuffle', fingerNote: "The card reversal must be completed while the right-hand packet is at its highest point — before it comes back down to the deck." },
      { _key: 'br-s3', stepNumber: 3, title: 'The Revelation', instruction: "Spread through the deck to find the face-up card. It appears to have turned itself over — an impossible event that requires no explanation because it cannot be explained.", cardAnimation: 'reveal' },
    ],
    commonMistakes: [
      { _key: 'br-m1', mistake: 'The reversed card flashing during replacement', fix: "The replacement of the right-hand packet must cover the reversed card completely. Practice the angle of replacement." },
      { _key: 'br-m2', mistake: 'Too much movement during the reversal', fix: "The motion should be minimal — the right hand lifts just enough to allow the left thumb to push the card. Large movements draw attention." },
    ],
    practiceDrill: { title: 'Blind Reversal', description: "Perform the Braue reversal with your eyes closed — relying entirely on feel. When you open your eyes and spread the deck, the card should be perfectly reversed.", reps: '30 blind reversals — zero mispositioning' },
    performanceContext: "The reversed card discovery is one of the most impossible-looking effects in card magic. A chosen card face-up in a face-down deck requires no explanation — it simply should not be possible.",
  },

  // CONTROLS — 4 more
  {
    _id: 'lesson-card-to-top-control',
    _type: 'lesson',
    title: 'Card to Top Control',
    slug: { _type: 'slug', current: 'card-to-top-control' },
    category: 'controls',
    level: 'beginner',
    tagline: "Their card goes back in the middle. It ends up on top. Every time.",
    overview: "The card-to-top control is the fundamental move of card magic — getting a spectator's chosen card from anywhere in the deck to the top position where you can reveal it. It is the most-performed technique in close-up card magic and must become absolutely automatic.",
    estimatedTime: 20,
    featured: false,
    order: 10,
    steps: [
      { _key: 'ct-s1', stepNumber: 1, title: 'The Return and Break', instruction: "Spread the deck for the spectator to return their card. As they place it in the spread, break the spread exactly there and let the card go in — then immediately get a pinky break above it as you square the halves together.", cardAnimation: 'fan', fingerNote: "The break must be taken in the same motion as the squaring. No separate, visible step." },
      { _key: 'ct-s2', stepNumber: 2, title: 'The Control', instruction: "With your pinky break holding the position of their card, perform a double undercut — two cuts that bring the card from anywhere in the deck to the top. The cuts look completely fair. The card is now on top.", cardAnimation: 'cut' },
      { _key: 'ct-s3', stepNumber: 3, title: 'Confirmation Action', instruction: "After the control, perform one more false cut or false shuffle as 'extra mixing' — then name their card as you turn over the top card. The combination of an apparently fair replacement, two cuts, and extra mixing makes the revelation feel impossible.", cardAnimation: 'reveal', fingerNote: "The extra mixing step is optional but adds a layer of impossibility. Only include it if your false cut/shuffle is clean." },
    ],
    commonMistakes: [
      { _key: 'ct-m1', mistake: 'Spectator noticing where the break is', fix: "The break should be invisible from the front. If you see the spectator's eyes tracking the break position, your break is too large." },
      { _key: 'ct-m2', mistake: 'Hesitating before the control', fix: "The control should begin the moment the deck is squared. Any pause between squaring and cutting reads as hesitation." },
    ],
    practiceDrill: { title: 'Instant Control', description: "Have a friend insert a card anywhere. Control it to the top without hesitation. They call a number (1–5) and you deal to that number — their card is there.", reps: '30 rounds — instant, smooth, zero hesitation' },
    performanceContext: "The card-to-top control is your utility move — it goes into every single routine. Practice it until it requires zero conscious thought. When performing, your mind should be on your audience, not on the control.",
  },

  {
    _id: 'lesson-herman-pass',
    _type: 'lesson',
    title: 'Herman Pass',
    slug: { _type: 'slug', current: 'herman-pass' },
    category: 'controls',
    level: 'advanced',
    tagline: 'Control a card to the bottom. Silent. Invisible. Instantaneous.',
    overview: "The Herman Pass (also called the side steal) moves a card from anywhere in the deck to the bottom in a single, completely invisible action disguised as a squaring gesture. It's an alternative to the classic pass for those who need a bottom control rather than a top control.",
    estimatedTime: 50,
    featured: false,
    order: 11,
    steps: [
      { _key: 'hp-s1', stepNumber: 1, title: 'The Break Position', instruction: "You need a pinky break above the target card. The card you want at the bottom is directly below your pinky break — you have a break above it.", cardAnimation: 'cut', fingerNote: "For the Herman Pass, the break position is opposite to most controls — you want the break ABOVE the target, not below." },
      { _key: 'hp-s2', stepNumber: 2, title: 'The Side Steal', instruction: "Your right hand comes over the deck in a squaring gesture. The right fingers contact the target card at its outer right corner, pulling it sideways out of the deck — it slides to the right, behind the right fingers, as the right hand moves away from the deck.", cardAnimation: 'shuffle', fingerNote: "The card slides horizontally, not vertically. The right fingers are the guide — the card travels along them as a rail." },
      { _key: 'hp-s3', stepNumber: 3, title: 'The Replacement', instruction: "The stolen card, now held behind the right fingers, is deposited on the bottom of the deck as the right hand returns to square the bottom. The card is now at the bottom, placed there in the same motion used to take it.", cardAnimation: 'cut' },
    ],
    commonMistakes: [
      { _key: 'hp-m1', mistake: 'The card catching on other cards during the slide', fix: "The card must slide cleanly. If it catches, you're at the wrong angle. The right fingers must guide the card in a perfectly horizontal plane." },
      { _key: 'hp-m2', mistake: 'Right hand moving too far from the deck', fix: "The right hand barely moves — the card travels to it. Large hand movements create visual disturbance." },
    ],
    practiceDrill: { title: 'Bottom Builder', description: "Control five different cards to the bottom one at a time using the Herman Pass. After five passes, the bottom five cards should be your five target cards in order.", reps: '20 five-card sequences — no positional errors' },
    performanceContext: "The Herman Pass enables effects where cards 'travel' to specific positions — particularly the bottom of the deck, which can then be loaded into a card box, dealt as the last card of a game, or produced from beneath the deck.",
  },

  {
    _id: 'lesson-spectator-cuts-aces',
    _type: 'lesson',
    title: 'Spectator Cuts the Aces',
    slug: { _type: 'slug', current: 'spectator-cuts-aces' },
    category: 'controls',
    level: 'intermediate',
    tagline: 'They cut the deck four times. They find the four Aces. Every time.',
    overview: "Spectator Cuts the Aces is a classic card miracle — the spectator cuts the deck into four packets themselves, and each packet has an Ace on top. It uses the simplest technique possible: proper setup and misdirection. The spectator does all the 'work' and still produces the Aces.",
    estimatedTime: 35,
    featured: true,
    order: 12,
    steps: [
      { _key: 'sca-s1', stepNumber: 1, title: 'The Setup', instruction: "Before beginning, secretly position all four Aces: Ace on top of the deck, Ace at position 14 from the top, Ace at position 27, Ace at position 40. This can be done during a previous routine or a preliminary false shuffle that maintains the stack.", cardAnimation: 'fan', fingerNote: "The positions (1, 14, 27, 40) divide the 52-card deck into four roughly equal quarters. Each cut will land near one Ace." },
      { _key: 'sca-s2', stepNumber: 2, title: 'The Four Cuts', instruction: "Ask the spectator to cut the deck into four roughly equal piles by making three cuts. Each time they cut approximately to the middle, they'll be cutting near one of your hidden Aces. Guide them gently: 'Somewhere in the middle, just wherever feels right.'", cardAnimation: 'cut' },
      { _key: 'sca-s3', stepNumber: 3, title: 'The Revelation', instruction: "Turn over the top card of each of the four packets simultaneously. The four Aces appear. The spectator did all the cutting — you touched nothing after the initial setup.", cardAnimation: 'reveal', fingerNote: "Build tension before the reveal. 'You cut the cards four times yourself. I never touched them. Turn over the top card of each pile.'" },
    ],
    commonMistakes: [
      { _key: 'sca-m1', mistake: "Spectator cuts unevenly — misses an Ace", fix: "If cuts are too uneven, gently offer to 'even them out a bit' with a small adjustment — which puts you back in control of where the Aces are." },
      { _key: 'sca-m2', mistake: 'Setup visible during earlier handling', fix: "The setup must be invisible. Do it during a casual overhand shuffle of the previous trick's deck, or while the spectator is looking at a different card." },
    ],
    practiceDrill: { title: 'Setup Speed', description: "Starting from a shuffled deck, set all four Aces to positions 1, 14, 27, 40 in under 30 seconds — without looking like you're doing anything unusual. Time yourself.", reps: '20 setups — under 30 seconds each, no suspicious behaviour' },
    performanceContext: "This is one of the cleanest 'impossible' tricks in card magic because the spectator physically performs the cuts. Your hands are away from the deck during the cutting phase. The revelation carries enormous impact precisely because you did nothing.",
  },

  {
    _id: 'lesson-mind-reading-force',
    _type: 'lesson',
    title: 'Mind-Reading Force',
    slug: { _type: 'slug', current: 'mind-reading-force' },
    category: 'controls',
    level: 'intermediate',
    tagline: 'They think of a card. You already know what it is.',
    overview: "The mind-reading force combines a standard force technique with the presentation frame of mind reading. The spectator believes they freely thought of a card — in reality, you forced it on them. This framing transforms a simple card trick into an apparently genuine psychic demonstration.",
    estimatedTime: 30,
    featured: false,
    order: 13,
    steps: [
      { _key: 'mrf-s1', stepNumber: 1, title: 'The Presentation Frame', instruction: "Establish the premise before revealing the method: tell the spectator you want to test whether you can influence their choices. Ask them to think of a number between 1 and 10. Whatever they say, bridge to: 'Now I want you to think of a playing card — let it come to you naturally.'", },
      { _key: 'mrf-s2', stepNumber: 2, title: 'The Force During the Fan', instruction: "As you fan the deck, the force card is at a position you control. Fan slowly, and as you pass the force card, pause very slightly and fan it more openly than adjacent cards — subliminally drawing the eye to it. Most spectators will name the card you paused on.", cardAnimation: 'fan', fingerNote: "The pause and open fan must be almost imperceptible. 'Almost' is the key word — it should be felt, not seen." },
      { _key: 'mrf-s3', stepNumber: 3, title: 'The Revelation', instruction: "Without touching the deck, name the card they 'freely thought of.' The fact that you didn't touch or deal the deck makes it feel genuinely psychic — there was no physical mechanism for you to know.", cardAnimation: 'reveal' },
    ],
    commonMistakes: [
      { _key: 'mrf-m1', mistake: 'The pause being too obvious', fix: "Test your fan force on 20 people without any verbal frame. If more than 15 choose your force card, your visual force is working. If fewer, the pause is too subtle — increase it slightly." },
      { _key: 'mrf-m2', mistake: 'No backup plan if they choose differently', fix: "Always have a secondary force method ready. If the fan force fails, transition immediately to a physical force (riffle or Hindu) framed as 'let me show you the deck.'" },
    ],
    practiceDrill: { title: 'Fan Force Rate', description: "Force the Ace of Spades on 20 different people using only the fan force — no physical touch. Track how many choose it. Target: 16+ of 20.", reps: '20-person sessions — track your success rate' },
    performanceContext: "The mind-reading frame elevates a card trick to a mentalism experience. The absence of physical dealing or cutting makes it feel qualitatively different from 'regular' card magic. This is the direction of professional close-up performance.",
  },

  {
    _id: 'lesson-black-and-red-separation',
    _type: 'lesson',
    title: 'Black and Red Separation',
    slug: { _type: 'slug', current: 'black-and-red-separation' },
    category: 'controls',
    level: 'advanced',
    tagline: 'Shuffle red and black cards together. They separate themselves.',
    overview: "A well-shuffled deck is spread face-up: all the red cards are on one side, all the black cards on the other. No sorting was seen — in fact the spectator shuffled the deck. This is one of the most baffling mathematical-seeming card effects, achieved through a combination of a Faro shuffle and a secret setup.",
    estimatedTime: 40,
    featured: false,
    order: 14,
    steps: [
      { _key: 'br-sep-s1', stepNumber: 1, title: 'The Si Stebbins Stack', instruction: "The separation relies on a deck arranged in a repeating colour pattern: RBRBRB... (Red-Black alternating throughout all 52 cards). This arrangement survives a single perfect Faro out-shuffle — the colours remain alternating. Prepare this secretly before the performance.", cardAnimation: 'fan', fingerNote: "The spectator never sees the deck face-up during setup. Handle it face-down at all times until the revelation." },
      { _key: 'br-sep-s2', stepNumber: 2, title: 'The Spectator Shuffle', instruction: "Ask the spectator to perform a table riffle shuffle — but secretly ensure it's a near-perfect Faro. Guide the split to be exactly equal. If done correctly, the alternating red-black pattern is maintained through the shuffle.", cardAnimation: 'shuffle', fingerNote: "You can offer to 'help them cut it in half evenly' — this is the real work. Getting equal halves ensures the Faro maintains the colour separation." },
      { _key: 'br-sep-s3', stepNumber: 3, title: 'The Revelation', instruction: "Spread the deck face-up across the table in a ribbon spread. The first card is red, the next black, alternating throughout the entire 52-card spread. Ask: 'What does that look like to you?' Let them see it themselves before explaining the impossibility.", cardAnimation: 'reveal' },
      { _key: 'br-sep-s4', stepNumber: 4, title: 'The Dramatic Version', instruction: "For a stronger revelation: push alternate cards (all reds or all blacks) to one side during the spread, physically separating the deck into two colour groups. The visual of 26 red cards and 26 black cards in two separate piles is stunning." },
    ],
    commonMistakes: [
      { _key: 'br-sep-m1', mistake: 'Spectator shuffles unevenly — pattern breaks', fix: "An uneven riffle breaks the alternating pattern at the junction point. Have a recovery plan: re-shuffle with a false shuffle to restore the pattern, framing it as 'just one more mix.'" },
      { _key: 'br-sep-m2', mistake: 'Deck seen face-up before revelation', fix: "If the spectator sees alternating red-black before you're ready to reveal, the method is exposed. Keep the deck face-down until the moment of revelation." },
    ],
    practiceDrill: { title: 'Pattern Survival Test', description: "Set up the alternating pattern. Have someone perform a table riffle shuffle. Spread and check: is the pattern intact? Test ten consecutive shuffles — track the failure rate and identify what breaks the pattern.", reps: '20 shuffle tests — understand the failure modes' },
    performanceContext: "Black and Red Separation plays as a mathematical miracle — not a card trick. Frame it as an experiment in order and chaos: 'No matter how thoroughly you mix them, order reasserts itself.' It reads as genuinely impossible.",
  },
]
